...
int main(int argc, char* argv[]) {
	...
	cout << "Point (2,2,0) inside Triangle 1: " << t1.pointInTriangle(new Point(2, 2, 0)) << endl;
	cout << "Height at Point (2,2,0) inside Triangle 1: " << t1.getHeightAtPoint(new Point(2, 2, 0)) << endl;
	cout << "Point (-2, -2, 0) inside Triangle 1: " << t1.pointInTriangle(new Point(-2, -2, 0)) << endl;
	cout << endl;
	cout << "Graph Tests: " << endl;
	cout << "Graph 1: 25 random points" << endl;
	for (int i = 0; i < 25; i++) {
		Point* new_point = new Point((rand() % 1000), (rand() % 700), (double)(rand() % 9) + 3);
	...
	/*for (int x = 0; x < 5; x++) {
		for (int y = 0; y < 5; y++) {
			Point* p = new Point(200 + (x * 80), 200 + (y * 80), 3);
			g2.addPoint(p);
		}
	}*/
	for (int i = 0; i < 25; i++) {
